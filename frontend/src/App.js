...
function App() {
  ...
  const sendMessage = async () => {
    ...
    setIsLoading(true); // Start loading
    console.log(groupId);
    //result.group_id || '2hfqPTgLJuE' || 'ottvOFTSGTw'
    try {
      const response = await fetch(
        `${API_ENDPOINT}/rag?group_id=${groupId || 'MDHIdAQMg-4'}`,
        {
          method: "POST",
      ...
      <div className={`app-container ${isLoading ? 'faded' : ''}`}>
        ...
        <div className="chat-container">
          <div className="input-area">
            <input type="file" accept="application/pdf, text/csv" onChange={handleFileChange} />
